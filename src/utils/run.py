...
torch.manual_seed(0)  # TODO: change to 42
...
if __name__ == "__main__":
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            rescaled_next_action = ((next_action_tensor + 1) / 2) * (
                action_space.high - action_space.low
            ) + action_space.low

            # Clip the rescaledaction to ensure it falls within the bounds of the action space
            clipped_next_action = torch.clamp(
                rescaled_next_action,
                torch.from_numpy(action_space.low),
                torch.from_numpy(action_space.high),
            )
            ...
            # Calculate advatage (temporal-difference)
            advantage = reward + gamma * (1 - terminated) * next_q_value - q_value
            ...
            actor_loss = (
                -action_distribution.log_prob(clipped_action)  # type: ignore
                * advantage
