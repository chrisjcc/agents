...
class Trainer: # responsible for running over the steps and collecting all the data
    ...
    def train_step(
        ...
        self.agent.update(state, clipped_action, reward, next_state, done)
    ...
    def train(self) -> List[int]:
        ...
        for episode in range(self.max_episodes):
            ...
            episode_reward = torch.FloatTensor([0]).unsqueeze(0).to(device)

            for step in range(self.max_steps):
                next_state, reward, done = self.train_step(state)
            ...
            if episode % self.checkpoint_freq == 0:
                self.save_checkpoint(self.checkpoint_path, episode, episode_reward.item())

            # Save last episode model
            self.save_checkpoint(
                self.checkpoint_path, self.max_episodes, episode_reward.item()
            )
